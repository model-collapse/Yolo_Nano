...
class YOLOLayer(nn.Module):
    ...
    def compute_grid_offsets(self, grid_size, cuda=True):
        ...
        self.grid_x = np.arange(g)[np.newaxis,:].repeat(g,0).astype(np.float).reshape(1,1,g,g)#torch.arange(g).repeat(g, 1).view([1, 1, g, g]).type(FloatTensor)
        self.grid_y = np.arange(g)[:,np.newaxis].repeat(g,1).astype(np.float).reshape(1,1,g,g)#torch.arange(g).repeat(g, 1).t().view([1, 1, g, g]).type(FloatTensor)
        ...
        self.scaled_anchors = FloatTensor(self.scaled_anchors)
    ...
    def forward(self, x, targets=None, img_dim=None,img_scores=None,gt_mix_index = None):
        ...
        self.img_dim = img_dim.cpu().numpy()
        num_samples = x.size(0).cpu().numpy()
        grid_size = x.size(2).cpu().numpy()
